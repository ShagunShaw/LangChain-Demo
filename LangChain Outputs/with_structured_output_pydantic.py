...
class DetailedReview(BaseModel):
    ...
    sentiment: Literal["pos", "neg", "neutral"] = Field(description="The overall sentiment of the review, e.g., positive, negative, neutral")
    pros: Optional[list[str]] = Field(default=None, description="A list of pros mentioned in the review, if any")
    cons: Optional[list[str]] = Field(default=None, description="A list of cons mentioned in the review, if any")       # Necessary to specify default value for Optional fields
...
print(result2.name)     # to get any particular field from the output, we can use dot notation as the output is now a pydantic model instance